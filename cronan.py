...
def initialize(bot_class=Bot, formatter_class=Formatter):
	...
	async def on_ready():
		...
		if bot.settings.token and not bot.settings.self_bot:
			...
			print("\nhttps://discordapp.com/oauth2/authorize?client_id=358033078696017930&scope=bot&permissions=470019190")
		...
		await bot.change_presence(game=discord.Game(name="out for ->help", type=3))
	...
	async def on_command_error(error, ctx):
		...
		if isinstance(error, commands.MissingRequiredArgument):
			...
			if not (is_forbidden and (channel.id == 300459722220503060)):
				bot.logger.exception("Exception in command '{}'".format(
					ctx.command.qualified_name), exc_info=error.original)
				message = ("I seem to have run into a problem. My owner either sees this or(if possible) you should message him about it. Most likely its just an error in ur command that my code doesnt know how to overcome.")
