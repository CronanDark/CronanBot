...
class Owner:
	...
	@checks.is_owner()
	@commands.command()
	async def msgchan(self, channelid, *, msg):
		""" hai"""
	...
	async def useravatar(self, ctx, user : discord.User=None):
		"""avatar"""
		if user is None:
	...
	async def join(self):
		...
		if self.bot.user.bot:
			await self.bot.whisper("Invite URL: https://discordapp.com/oauth2/authorize?client_id=358033078696017930&scope=bot&permissions=470019190")
		else:
	...
	async def serverroles(self, ctx):
		server = ctx.message.server
		roles = server.roles
		rolebed = discord.Embed(color=discord.Color.red())
		rolelist = ""
		try:
		...
		rolebed.add_field(name="Server Roles:", value=rolelist)

		await self.bot.say(embed=rolebed)
	...
	async def serverowner(self, ctx, servername):
		...
		await self.bot.say(correctserv.owner.id)
